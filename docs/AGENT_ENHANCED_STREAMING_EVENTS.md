...
const StepResultViewer: React.FC<{ messages: Message[] }> = ({ messages }) => {
  const [viewMode, setViewMode] = useState<'formatted' | 'raw' | 'both'>('formatted');
  
  // 按步骤分组消息
  const stepResults = messages.reduce((acc, msg) => {
    const stepNumber = msg.metadata?.stepNumber;
    if (!stepNumber) return acc;
    
    if (!acc[stepNumber]) {
      acc[stepNumber] = {
        stepNumber,
        toolName: msg.metadata.stepName,
        mcpName: msg.metadata.toolDetails?.mcpName,
        toolDetails: msg.metadata.toolDetails
      };
    }
    ...
    if (msg.metadata.contentType === 'raw_result') {
      acc[stepNumber].raw = {
        content: msg.content,
        data: msg.metadata.executionDetails?.rawResult,
        timestamp: msg.createdAt
      };
    } else if (msg.metadata.contentType === 'formatted_result') {
      acc[stepNumber].formatted = {
        content: msg.content,
        timestamp: msg.createdAt
      };
    }
  ...
  return (
    <div className="step-results">
      ...
      {Object.values(stepResults).map(step => (
        <div key={step.stepNumber} className="step-result">
          <div className="step-header">
            <h3>Step {step.stepNumber}: {step.toolName}</h3>
            {step.mcpName && <span className="mcp-badge">{step.mcpName}</span>}
            <span className="tool-type">{step.toolDetails.toolType.toUpperCase()}</span>
          </div>
          ...
          <div className="step-details">
            <details>
              ...
              <div className="tool-info">
                ...
                <pre>{JSON.stringify(step.toolDetails.args, null, 2)}</pre>
                <p><strong>预期输出:</strong> {step.toolDetails.expectedOutput}</p>
                <p><strong>推理:</strong> {step.toolDetails.reasoning}</p>
              </div>
            </details>
          </div>

          <div className="step-content">
            {(viewMode === 'formatted' || viewMode === 'both') && step.formatted && (
              <div className="formatted-result">
                <h4>📄 格式化结果</h4>
                <div className="content">{step.formatted.content}</div>
                <small className="timestamp">
                  {new Date(step.formatted.timestamp).toLocaleString()}
                </small>
              </div>
            )}
            
            {(viewMode === 'raw' || viewMode === 'both') && step.raw && (
              <div className="raw-result">
                <h4>🔧 原始数据</h4>
                <pre className="json-content">{step.raw.content}</pre>
                <small className="timestamp">
                  {new Date(step.raw.timestamp).toLocaleString()}
                </small>
              </div>
            )}
          </div>
...
.formatted-result, .raw-result {
  margin-bottom: 16px;
}

.formatted-result h4, .raw-result h4 {
  margin: 0 0 12px 0;
  color: #495057;
}
