            ...
            if (content.text) {
              try {
                ...
                for (const pattern of patterns) {
                  ...
                  if (match) {
                    draftId = match[1];
                    logger.info(`📝 X-MCP Auto-publish: Extracted draft_id "${draftId}" using pattern: ${pattern}`);
                    break;
                  }
                }
      ...
      if (!draftId && typeof result === 'string') {
        ...
        try {
          ...
          for (const pattern of patterns) {
            ...
            if (match) {
              draftId = match[1];
              logger.info(`📝 X-MCP Auto-publish: Extracted draft_id "${draftId}" from string using pattern: ${pattern}`);
              break;
            }
          }
        }
